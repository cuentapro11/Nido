...
export default function ActivityModal({ open, onClose, preselectedChildId }: Props) {
  ...
  const { show } = useToast();
  const [step, setStep] = useState<'pick' | 'child' | 'confirm' | 'no-entry'>('pick');
  ...
  const handlePickOpt = (opt: typeof LOG_OPTS[0]) => {
    ...
    if (preselectedChildId != null) {
      const ch = children.find(c => c.id === preselectedChildId);
      setSelectedChildId(preselectedChildId);

      // Si es entrada o salida, permitir siempre
      if (opt.type === 'entry' || opt.type === 'exit') {
        setStep('confirm');
        return;
      }

      // Para cualquier otra actividad, verificar que tiene entrada
      if (ch && !ch.present) {
        setStep('no-entry');
        return;
      }
  ...
  // Quién puede recibir esta actividad según el tipo
  const candidatos = selectedOpt?.type === 'entry' ? ausentes
    : selectedOpt?.type === 'exit' ? presentes
    : presentes;

  const filtered = q ? candidatos.filter(c => c.name.toLowerCase().includes(q)) : candidatos;
  ...
  return (
    <BottomSheet open={open} onClose={reset}>
            ...
            {filtered.length === 0 && (
              <div className="text-center py-6 text-muted-foreground">
                <p className="text-sm font-bold text-foreground">
                  {selectedOpt?.type === 'entry' ? 'Todos los niños ya tienen entrada' :
                   selectedOpt?.type === 'exit'  ? 'No hay niños con entrada activa' :
                   'No hay niños con entrada. Registra la entrada primero.'}
                </p>
              </div>
            )}
            {filtered.map(c => (
      ...
      {step === 'no-entry' && selectedChildId != null && (() => {
        const ch = children.find(c => c.id === selectedChildId);
        if (!ch) return null;
        return (
          <div className="flex flex-col items-center text-center">
            ...
            <h3 className="text-lg font-extrabold text-foreground">Sin entrada registrada</h3>
            <p className="text-sm text-muted-foreground mt-2 leading-relaxed mb-6">
              <span className="font-semibold text-foreground">{ch.name.split(' ')[0]}</span> no tiene entrada registrada hoy.
              Para registrar actividades debes registrar la entrada primero.
            </p>
            <button
              onClick={() => {
                // Registrar entrada automáticamente
                togglePresence(selectedChildId);
                show(`✓ Entrada de ${ch.name.split(' ')[0]} registrada`);
                // Volver a pick para elegir la actividad ahora
                setStep('pick');
                setSelectedOpt(null);
              }}
              className="w-full bg-primary text-primary-foreground font-bold py-3.5 rounded-2xl text-sm active:scale-[0.98] mb-2"
            >
              Registrar entrada ahora
            </button>
            <button
              onClick={() => setStep('pick')}
              className="w-full border border-border text-muted-foreground font-semibold py-3 rounded-2xl text-sm"
            >
              Cancelar
            </button>
          </div>
