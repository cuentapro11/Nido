...
function EditarAula({ aula, onBack, onDeleted }: {
  ...
  return (
    <SlideScreen>
      <div className="flex items-center gap-3 px-4 pt-10 pb-4 border-b border-border bg-background flex-shrink-0">
...
export default function Aulas() {
  ...
  const [searchNinos, setSearchNinos] = useState('');
  const [searchCuids, setSearchCuids] = useState('');

  // Al cambiar categoría, pre-llena el nombre con la categoría elegida
  ...
  const GROUP_ORDER = ['Bebés', 'Caminadores', 'Pre-escolar'];
  const sinAulaList = children
    .filter(c => !aulaAsignaciones[c.id] || aulaAsignaciones[c.id] === -1)
    .sort((a, b) => {
  ...
  const openCreate = () => {
    const initCat = getCategoryForIndex(aulas.length);
    setNewNombre('');
    setNewCategoria(initCat);
    setNewCustomCat('');
    setNewNinos([]);
    setNewCuids([]);
    setSearchNinos('');
  ...
  return (
    <div className="min-h-screen bg-background pb-8">
      ...
      {/* Header */}
      <div className="flex items-center gap-3 px-4 pt-10 pb-4 border-b border-border bg-card">
      ...
      <div className="px-4 space-y-2.5">
        {aulas.length === 0 ? (
          ...
          .slice()
          .sort((a, b) => {
            const order = ['Bebés', 'Caminadores', 'Pre-escolar'];
            const catA = (a as any).categoria || (a as any).nombre || '';
            const catB = (b as any).categoria || (b as any).nombre || '';
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
                {/* Categoría — primero para que auto-llene el nombre */}
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button type="button" onClick={() => handleCategoriaChange('__custom__')}
                      className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all active:scale-95 ${
                ...
                {/* Nombre — se pre-llena con la categoría */}
                <div>
                  ...
                  <input value={newNombre} onChange={e => setNewNombre(e.target.value)}
                    placeholder={newCategoria && newCategoria !== '__custom__' ? `Ej: ${newCategoria} - Ana Luz` : 'Ej: Bebés - Ana Luz'}
                  ...
                  {newCategoria && newCategoria !== '__custom__' && (
                    <p className="text-[11px] text-muted-foreground mt-1.5 px-1">
                      💡 Puedes agregar un diferenciador, ej: <span className="font-bold text-foreground">{newCategoria} - Ana Luz</span>
                    </p>
                  )}
                </div>
                ...
                {/* Cuidadores en creación — primero, con búsqueda, todos disponibles */}
                <div>
                  <div className="flex items-center gap-2 mb-3">
                  ...
                  {cuidadores.length === 0 ? (
                    ...
                    <>
                      <div className="relative mb-2">
                        ...
                        <input value={searchCuids} onChange={e => setSearchCuids(e.target.value)}
                          placeholder="Buscar cuidador..."
                          className="w-full bg-background border border-border rounded-xl pl-8 pr-4 py-2 text-sm text-foreground outline-none focus:border-primary" />
                      </div>
                      <div className="space-y-1.5">
                        {cuidadores
                          .filter(c => c.nombre.toLowerCase().includes(searchCuids.toLowerCase()))
                          .map(c => {
                ...
                {/* Niños en creación — solo sin aula, con búsqueda */}
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <Baby size={13} className="text-muted-foreground" />
                    <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Niños sin sala</span>
                    {newNinos.length > 0 && (
                      <span className="ml-auto bg-primary/15 text-primary text-[10px] font-extrabold px-2 py-0.5 rounded-full">
                  ...
                  {(() => {
                    const sinAula = children.filter(c => !aulaAsignaciones[c.id] || aulaAsignaciones[c.id] === -1);
                    const filtrados = sinAula.filter(c => c.name.toLowerCase().includes(searchNinos.toLowerCase()));
                    if (sinAula.length === 0) return <p className="text-sm text-muted-foreground italic">Todos los niños ya tienen sala asignada</p>;
                    return (
                      <>
                        <div className="relative mb-2">
                          <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
                          <input value={searchNinos} onChange={e => setSearchNinos(e.target.value)}
                        ...
                        <div className="space-y-1.5">
                          ...
                          {filtrados.length === 0 && sinAula.length > 0 && (
